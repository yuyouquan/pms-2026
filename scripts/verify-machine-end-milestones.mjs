...
await machine.persist.rehydrate()
assert.equal(latest(budgetId).milestones.str5,'2027-08-31','bound budget keeps its saved source date')
assert.equal(latest(budgetId).milestones.str5Plus6Months,'2028-02-29','bound budget rejects derived-date writes')
machine.getState().updateVersion(record(budgetId).id,latest(budgetId).id,{milestones:{str5:null}})
assert.equal(latest(budgetId).milestones.str5Plus6Months,'2028-02-29','bound budget also rejects clearing the source date')
