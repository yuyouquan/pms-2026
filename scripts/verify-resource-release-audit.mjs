...
assert.equal(Object.values(monthly().monthlyData).reduce((sum,value)=>sum+value,0),6,'upper investment changes recalculate monthly allocation')
assert.equal(monthly().isEdited,false)
capability.getState().updateMonthlyInvestment(monthly().id, { '2027-01': 8, '2027-02': 4 })
await capability.persist.rehydrate()
assert.equal(capability.getState().projects.find(row=>row.id===p.id).versions.find(row=>row.id===v.id).estimatedInvestment,6,'monthly adjustment never changes upper estimate')
...
assert.equal(latest.estimatedInvestment, first.estimatedInvestment, 'saved versions retain their selected model snapshots')
machine.getState().updateVersionInline(mp.id, latest.id, {type:'model',key:'levelCoefficient',value:2},mp.pmsProjectId)
machine.getState().updateVersionInline(mp.id, latest.id, {type:'model',key:'levelCoefficient',value:1},mp.pmsProjectId)
assert.equal(versions().at(-1).estimatedInvestment, first.estimatedInvestment + 10, 'explicit selection uses updated configuration')
...
console.log('PASS: explicit model reselection uses config edits; passive refresh and retirement preserve saved snapshots')
