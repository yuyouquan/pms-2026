...
capability.getState().setVersionLocked(cap().id, latestId, true)
capability.getState().updateVersion(cap().id,latestId,{projectStartTime:'2040-01-01'})
assert.equal(cap().versions.find(v=>v.id===latestId).projectStartTime,'2031-03-01','locked history remains immutable')
assert.equal(newVersion().projectStartTime,'2031-03-01','copy inherits independent dates')
console.log('PASS capability formal manual create/edit/copy/reload dates and version lock guard')
