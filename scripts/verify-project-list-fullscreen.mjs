...
assert.match(source, /projectListView !== 'card'[\s\S]{0,400}aria-label="全屏展示"[\s\S]{0,120}icon=\{<FullscreenOutlined \/>\}[\s\S]{0,120}setIsFullscreen\(true\)/)
assert.match(source, /aria-label="退出全屏"[\s\S]{0,120}icon=\{<FullscreenExitOutlined \/>\}[\s\S]{0,120}setIsFullscreen\(false\)/)
