...
assert.match(browserSource, /ALLOWED_BROWSER_WARNINGS/, 'browser warning handling must use an explicit allowlist')
assert.match(browserSource, /message\.type\(\)\s*===\s*['"]warn['"]/, 'browser warnings must pass through the explicit warning gate')
assert.doesNotMatch(browserSource, /browser\?\.close\(\)\.catch/, 'browser close failures must not be swallowed')
assert.match(browserSource, /\[cleanup\][\s\S]{0,180}process\.exitCode\s*=\s*1/, 'browser close failures must make verification fail')
