...
assert.deepEqual(
  ...
  ['2026-09-01', '2026-09-03', 2, '2026-09-02', '2026-09-05', 3, false],
  'business periods preserve ranges and use end-minus-start planned and actual durations',
)
assert.deepEqual(
  ...
  ['2026-09-01', '2026-09-03', 2, '2026-09-02', '2026-09-05', 3, false],
  'stages derive end-minus-start schedule ranges from their children',
)
const tenDayBusinessProjection = level1Rules.projectLevel1Plan([
  { id: 'ten-day-stage', stableId: 'ten-day-stage', order: 1, taskName: '十天阶段', nodeKind: 'stage' },
  { id: 'ten-day-period', stableId: 'ten-day-period', parentId: 'ten-day-stage', order: 1, taskName: 'MR10', nodeKind: 'business-period', planStartDate: '2026-03-01', planEndDate: '2026-03-11', actualStartDate: '2026-03-02', actualEndDate: '2026-03-12' },
], { mode: 'standard' })
assert.deepEqual(
  tenDayBusinessProjection.rows.map(row => [row.id, row.estimatedDays, row.actualDays]),
  [['ten-day-stage', 10, 10], ['ten-day-period', 10, 10]],
  'a March 1 to March 11 business period and its parent stage both have ten planned and actual days',
)

const emptyFirstStageProjection = level1Rules.projectLevel1Plan([
  { id: 'empty-first-stage', stableId: 'empty-first-stage', order: 1, taskName: '空阶段', nodeKind: 'stage' },
  { id: 'empty-first-fixed', stableId: 'empty-first-fixed', parentId: 'empty-first-stage', order: 1, taskName: '空节点', nodeKind: 'fixed-milestone' },
  { id: 'scheduled-second-stage', stableId: 'scheduled-second-stage', order: 2, taskName: '有日期阶段', nodeKind: 'stage' },
  { id: 'scheduled-second-fixed', stableId: 'scheduled-second-fixed', parentId: 'scheduled-second-stage', order: 1, taskName: '有日期节点', nodeKind: 'fixed-milestone', planEndDate: '2026-10-08', actualEndDate: '2026-10-09' },
], { mode: 'standard', today: '2026-10-10' })
assert.deepEqual(
  emptyFirstStageProjection.rows.filter(row => !row.parentId).map(row => [row.id, row.planStartDate, row.planEndDate, row.actualStartDate, row.actualEndDate]),
  [
    ['empty-first-stage', '', '', '', ''],
    ['scheduled-second-stage', '2026-10-08', '2026-10-08', '2026-10-09', '2026-10-09'],
  ],
  'an empty first stage stays empty and the first scheduled later stage begins at its own first fixed completion',
)
...
assert.deepEqual(
  ...
  ['2026-01-10', '2026-01-20', 10, '2026-01-11', '2026-01-21', 10],
...
const equalFixedValidation = level1Rules.validateLevel1ScheduleDates(equalFixedDates)
assert.equal(equalFixedValidation.valid, false, 'fixed milestones reject equal planned and actual completion dates')
assert.deepEqual(equalFixedValidation.byTaskId['equal-b']?.planEndDate, ['下一个子节点日期不允许超上一个子节点。'], 'same-day planned fixed milestones use the exact order error')
assert.deepEqual(equalFixedValidation.byTaskId['equal-b']?.actualEndDate, ['下一个子节点日期不允许超上一个子节点。'], 'same-day actual fixed milestones use the exact order error')
...
assert.equal(equalCrossStageFixedValidation.valid, false, 'fixed milestones reject equal planned and actual completion points across stage boundaries')
assert.ok(equalCrossStageFixedValidation.byTaskId['equal-stage-2-fixed']?.planEndDate?.every(message => message === '下一个子节点日期不允许超上一个子节点。'), 'cross-stage planned equality uses the exact order error')
assert.ok(equalCrossStageFixedValidation.byTaskId['equal-stage-2-fixed']?.actualEndDate?.every(message => message === '下一个子节点日期不允许超上一个子节点。'), 'cross-stage actual equality uses the exact order error')
...
assert.ok(reversedFixedValidation.byTaskId['fixed-b']?.actualEndDate?.every(message => message === '下一个子节点日期不允许超上一个子节点。'), 'same-stage actual reversal identifies the later fixed milestone with the exact order error')
assert.ok(reversedFixedValidation.byTaskId['fixed-c']?.planEndDate?.every(message => message === '下一个子节点日期不允许超上一个子节点。'), 'cross-stage planned reversal identifies the later fixed milestone with the exact order error')
...
assert.deepEqual(overlappingBusinessValidation.byTaskId['period-b']?.planStartDate, ['下一个子节点日期不允许超上一个子节点。'], 'same-day planned business boundaries use the exact order error')
assert.deepEqual(overlappingBusinessValidation.byTaskId['period-b']?.actualStartDate, ['下一个子节点日期不允许超上一个子节点。'], 'actual business periods validate independently with the exact order error')

const independentScheduleAxes = overlappingBusinessPeriods.map(task => task.id === 'period-b'
  ? { ...task, actualStartDate: '2026-04-12', actualEndDate: '2026-04-21' }
  : task)
const independentScheduleValidation = level1Rules.validateLevel1ScheduleDates(independentScheduleAxes)
assert.deepEqual(independentScheduleValidation.byTaskId['period-b']?.planStartDate, ['下一个子节点日期不允许超上一个子节点。'], 'an invalid planned sequence remains reported')
assert.equal(independentScheduleValidation.byTaskId['period-b']?.actualStartDate, undefined, 'a valid actual sequence stays independent from an invalid planned sequence')
...
assert.deepEqual(longRunningBusinessValidation.byTaskId['long-period-c']?.planStartDate, ['下一个子节点日期不允许超上一个子节点。'], 'planned overlap detection retains an earlier long-running interval beyond its immediate neighbor')
assert.deepEqual(longRunningBusinessValidation.byTaskId['long-period-c']?.actualStartDate, ['下一个子节点日期不允许超上一个子节点。'], 'actual overlap detection retains an earlier long-running interval beyond its immediate neighbor')
...
assert.deepEqual(overlappingStageValidation.byTaskId['overlap-period-2']?.planStartDate, ['下一个子节点日期不允许超上一个子节点。'], 'derived stage overlap maps the exact order error to the editable child boundary field')
...
assert.deepEqual(longRunningStageValidation.byTaskId['short-stage-period-c']?.planStartDate, ['下一个子节点日期不允许超上一个子节点。'], 'planned stage overlap retains an earlier long-running stage beyond its immediate neighbor')
assert.deepEqual(longRunningStageValidation.byTaskId['short-stage-period-c']?.actualStartDate, ['下一个子节点日期不允许超上一个子节点。'], 'actual stage overlap retains an earlier long-running stage beyond its immediate neighbor')
...
assert.deepEqual(unorderedStageBoundaryValidation.byTaskId['unordered-next-period']?.planStartDate, ['下一个子节点日期不允许超上一个子节点。'], 'planned stage overlap uses the maximum valid child end instead of the last displayed child')
assert.deepEqual(unorderedStageBoundaryValidation.byTaskId['unordered-next-period']?.actualStartDate, ['下一个子节点日期不允许超上一个子节点。'], 'actual stage overlap uses the maximum valid child end instead of the last displayed child')
...
const reorderFixture = [
  { id: '1', stableId: 'business-root', order: 1, taskName: '上市阶段', source: 'template', nodeKind: 'stage' },
  { id: '1.1', stableId: 'mr-early', parentId: '1', order: 1, taskName: 'MR1', source: 'custom', nodeKind: 'business-period', planStartDate: '2026-03-01', planEndDate: '2026-03-11', actualStartDate: '2026-03-02', actualEndDate: '2026-03-12' },
  { id: '1.2', stableId: 'mr-late', parentId: '1', order: 2, taskName: 'MR2', source: 'custom', nodeKind: 'business-period', planStartDate: '2026-03-12', planEndDate: '2026-03-22', actualStartDate: '2026-03-13', actualEndDate: '2026-03-23' },
]
const reorderSnapshot = structuredClone(reorderFixture)
const invalidReorder = level1Rules.reorderLevel1BusinessNodes(reorderFixture, 'mr-early', 'mr-late')
assert.deepEqual(invalidReorder, { ok: false, message: '下一个子节点日期不允许超上一个子节点。' }, 'an invalid business reorder returns the exact order error')
assert.deepEqual(reorderFixture, reorderSnapshot, 'an invalid business reorder does not mutate its input')

const reorderableFixture = [reorderFixture[0], { ...reorderFixture[2], order: 1 }, { ...reorderFixture[1], order: 2 }]
const validReorder = level1Rules.reorderLevel1BusinessNodes(reorderableFixture, 'mr-early', 'mr-late')
assert.equal(validReorder.ok, true, 'a business reorder succeeds when the resulting planned and actual order is valid')
if (validReorder.ok) {
  assert.deepEqual(validReorder.tasks.filter(task => task.parentId).map(task => [task.stableId, task.id, task.order]), [
    ['mr-early', '1.1', 1],
    ['mr-late', '1.2', 2],
  ], 'a valid business reorder renumbers sibling display IDs and order')
}
...
assert.deepEqual(
  ...
  [
    ...
    ['mixed-period', 'task', '2026-04-01', '2026-04-10', 9],
  ],
  'hierarchical gantt emits fixed completion points and end-minus-start business-period bars',
)
assert.deepEqual(
  mixedGanttTasks.find(task => task.id === 'partial-gantt-period'),
  { ...mixedGanttSource[4], type: 'task', readonly: true, start_date: '', end_date: '', duration: 0, unscheduled: true },
  'an incomplete business period does not fabricate an editable gantt bar',
)
...
assert.deepEqual(businessPeriodMoved.find(task => task.id === 'mixed-period'), {
  ...mixedGanttSource[2], planStartDate: '2026-04-02', planEndDate: '2026-04-11', estimatedDays: 9,
}, 'business-period dragging patches both planned boundaries and recalculates an end-minus-start duration')
...
assert.deepEqual(businessActualPatched.find(task => task.id === 'mixed-period'), {
  ...mixedGanttSource[2], actualStartDate: '2026-04-03', actualEndDate: '2026-04-12', actualDays: 9,
}, 'business-period actual patches retain actual semantics and recalculate an end-minus-start duration')
...
const invalidFixedSequenceSource = [
  { id: 'invalid-stage', order: 1, taskName: '概念阶段', nodeKind: 'stage' },
  { id: 'invalid-fixed-a', parentId: 'invalid-stage', order: 1, taskName: '概念启动', nodeKind: 'fixed-milestone', planEndDate: '2026-04-05' },
  { id: 'invalid-fixed-b', parentId: 'invalid-stage', order: 2, taskName: 'STR1', nodeKind: 'fixed-milestone', planEndDate: '2026-04-10' },
]
assert.strictEqual(ganttRules.applyPlanGanttDateChange(invalidFixedSequenceSource, {
  taskId: 'invalid-fixed-b', mode: 'milestone', startDate: '2026-04-05', endDate: '2026-04-05',
}), invalidFixedSequenceSource, 'a same-day fixed milestone gantt update is rejected before any caller can write it')
assert.deepEqual(ganttRules.applyPlanGanttDateChangeResult(invalidFixedSequenceSource, {
  taskId: 'invalid-fixed-b', mode: 'milestone', startDate: '2026-04-05', endDate: '2026-04-05',
}), {
  ok: false,
...
const invalidCrossStageSource = [
  ...
  { id: 'cross-stage-b', order: 2, taskName: '计划阶段', nodeKind: 'stage' },
  { id: 'cross-fixed-b', parentId: 'cross-stage-b', order: 1, taskName: 'STR2', nodeKind: 'fixed-milestone', planEndDate: '2026-04-12' },
]
assert.strictEqual(ganttRules.applyPlanGanttDateChange(invalidCrossStageSource, {
  taskId: 'cross-fixed-b', mode: 'milestone', startDate: '2026-04-10', endDate: '2026-04-10',
}), invalidCrossStageSource, 'a cross-stage overlap is rejected by the shared schedule validation')
...
const ganttRulesSource = fs.readFileSync(path.join(root, 'src/lib/planGanttRules.ts'), 'utf8')
...
assert.match(ganttRulesSource, /onBeforeTaskDrag/, 'gantt blocks forbidden drag attempts before they change task dates')
assert.match(ganttRulesSource, /onBeforeTaskChanged/, 'gantt validates proposed dates before dhtmlx commits the drag')
assert.match(ganttRulesSource, /onAfterTaskDrag/, 'gantt records accepted task-date drags')
assert.match(ganttRulesSource, /onBeforeTaskUpdate/, 'gantt blocks readonly or project updates even when an update event bypasses drag')
assert.match(ganttRulesSource, /onBeforeLightbox/, 'gantt blocks project and readonly task editing in the lightbox')
assert.match(ganttRulesSource, /handlers\.forEach\(handler => host\.detachEvent\(handler\)\)/, 'gantt detaches every interaction lifecycle guard during cleanup')
...
assert.match(ganttHelperSource, /attachPlanGanttInteractionLifecycle/, 'gantt installs the tested real lifecycle event set')
...
;[rootTask, readonlyTask, editableTask].forEach(task => interactionController.rememberTask(task))
assert.equal(interactionController.beforeUpdate(rootTask), false, 'project roots cannot be updated through a non-drag dhtmlx event')
assert.equal(interactionController.beforeUpdate(readonlyTask), false, 'readonly tasks cannot be updated through a non-drag dhtmlx event')
assert.equal(interactionController.beforeUpdate(editableTask), false, 'editable leaves cannot bypass the accepted drag path with a standalone update event')
...
const updatesBeforeRejectedChange = updates.length
...
assert.equal(interactionController.beforeTaskChanged(editableTask, { ...editableTask, start_date: new Date('2026-06-01T00:00:00Z'), end_date: new Date('2026-06-01T00:00:00Z') }), false, 'rejected changes stop before dhtmlx commits task updates')
assert.deepEqual(latestCallbackPayload, { taskId: 'editable', nodeType: 'milestone', startDate: '2026-06-04', endDate: '2026-06-04' }, 'before-change reads the latest callback and formats proposed dates')
...
assert.equal(updates.length, updatesBeforeRejectedChange + 1, 'a false callback refreshes the restored task exactly once')
...
assert.equal(interactionController.beforeTaskChanged(editableTask, { ...editableTask, start_date: new Date('2026-06-01T00:00:00Z'), end_date: new Date('2026-06-01T00:00:00Z') }), true, 'accepted changes pass before dhtmlx commits task updates')
assert.equal(interactionController.beforeUpdate(editableTask), true, 'the accepted drag may pass the dhtmlx update gate')
...
assert.equal(updates.length, updatesBeforeRejectedChange + 1, 'a true callback does not issue a restore update')
currentDateChangeCallback = () => false
editableTask.start_date = new Date('2026-06-06T00:00:00Z')
assert.equal(interactionController.beforeUpdate(editableTask), false, 'standalone updates remain blocked after a completed drag')
assert.equal(asYmd(editableTask.start_date), '2026-06-05', 'blocked standalone updates restore the latest committed snapshot')
assert.equal(updates.length, updatesBeforeRejectedChange + 2, 'blocked standalone updates refresh the restored task once')
let bypassCallbackCalls = 0
currentDateChangeCallback = () => { bypassCallbackCalls += 1; return true }
interactionController.afterDrag(rootTask)
assert.equal(bypassCallbackCalls, 0, 'an after-drag event without an accepted before-drag snapshot cannot bypass project readonly')
interactionController.clear()
interactionController.afterDrag(editableTask)
assert.equal(updates.length, updatesBeforeRejectedChange + 2, 'clear prevents stale restore updates')
...
assert.equal(interactionController.canOpenLightbox(editableTask), false, 'editable leaves use validated drag or resize instead of the bypass-prone lightbox')
const technicalInteractionController = ganttRules.createPlanGanttInteractionController({ readOnly: false, getOnTaskDateChange: () => currentDateChangeCallback, formatDate: asYmd, updateTask: () => {} })
assert.equal(technicalInteractionController.beforeUpdate(editableTask), true, 'technical subproject gantt keeps its existing editable update semantics by default')
assert.equal(technicalInteractionController.canOpenLightbox(editableTask), true, 'technical subproject gantt keeps its existing editable lightbox semantics by default')
...
assert.equal(globallyReadonlyController.beforeUpdate(editableTask), false, 'global readonly blocks update events')
assert.equal(globallyReadonlyController.canOpenLightbox(editableTask), false, 'global readonly blocks lightbox editing')

const realGantt = require('dhtmlx-gantt').gantt
assert.equal(require('dhtmlx-gantt/package.json').version, '9.1.2', 'real lifecycle coverage runs against the repository dhtmlx version')
realGantt.clearAll()
const realLifecycleSource = [
  { id: 'real-stage', order: 1, taskName: '上市阶段', nodeKind: 'stage', planStartDate: '2026-07-01', planEndDate: '2026-07-31' },
  { id: 'real-period-a', parentId: 'real-stage', order: 1, taskName: 'MR1', nodeKind: 'business-period', planStartDate: '2026-07-01', planEndDate: '2026-07-10' },
  { id: 'real-period-b', parentId: 'real-stage', order: 2, taskName: 'MR2', nodeKind: 'business-period', planStartDate: '2026-07-11', planEndDate: '2026-07-20' },
]
const realProjectedTasks = ganttRules.buildPlanGanttTasks(realLifecycleSource, { mode: 'hierarchical', editable: true })
realGantt.parse({ data: realProjectedTasks.map(task => ({
  ...task,
  text: task.taskName,
  start_date: task.start_date ? new Date(`${task.start_date}T00:00:00Z`) : null,
  end_date: task.end_date ? new Date(`${task.end_date}T00:00:00Z`) : null,
})) })
let realStoreTasks = realLifecycleSource
let realStoreCallbackCalls = 0
let realValidationCalls = 0
let realPendingTasks = null
const realController = ganttRules.createPlanGanttInteractionController({
  readOnly: false,
  allowLightbox: false,
  allowStandaloneUpdate: false,
  getValidateTaskDateChange: () => change => {
    realValidationCalls += 1
    const result = ganttRules.applyPlanGanttDateChangeResult(realStoreTasks, { ...change, mode: change.nodeType })
    if (!result.ok) return false
    realPendingTasks = result.tasks
    return true
  },
  getOnTaskDateChange: () => () => {
    realStoreCallbackCalls += 1
    realStoreTasks = realPendingTasks
    realPendingTasks = null
    return true
  },
  formatDate: asYmd,
  updateTask: task => realGantt.updateTask(task.id),
  refreshTask: task => realGantt.refreshTask(task.id),
})
const detachRealLifecycle = ganttRules.attachPlanGanttInteractionLifecycle(realGantt, realController)
const runRealDragLifecycle = (taskId, startDate, endDate) => {
  const task = realGantt.getTask(taskId)
  const original = realGantt.copy(task)
  if (!realGantt.callEvent('onBeforeTaskDrag', [taskId, 'move', {}])) return false
  task.start_date = new Date(`${startDate}T00:00:00Z`)
  task.end_date = new Date(`${endDate}T00:00:00Z`)
  const accepted = realGantt.callEvent('onBeforeTaskChanged', [taskId, 'move', original, {}])
  if (accepted) {
    realGantt.updateTask(taskId)
    realGantt.callEvent('onAfterTaskDrag', [taskId, 'move', {}])
  }
  return accepted
}
const dirtyStage = realGantt.getTask('real-stage')
const originalStageStart = asYmd(dirtyStage.start_date)
dirtyStage.start_date = new Date('2030-01-01T00:00:00Z')
assert.equal(realGantt.callEvent('onBeforeTaskUpdate', ['real-stage', dirtyStage]), false, 'real dhtmlx rejects a standalone readonly stage update')
assert.equal(asYmd(realGantt.getTask('real-stage').start_date), originalStageStart, 'real dhtmlx readonly rejection restores the committed stage date immediately')
assert.equal(realStoreCallbackCalls, 0, 'readonly and standalone rejection never invokes the store callback')
const dirtyStandalonePeriod = realGantt.getTask('real-period-b')
dirtyStandalonePeriod.start_date = new Date('2026-07-09T00:00:00Z')
assert.equal(realGantt.callEvent('onBeforeTaskUpdate', ['real-period-b', dirtyStandalonePeriod]), false, 'real dhtmlx rejects an editable standalone update outside the accepted drag lifecycle')
assert.equal(asYmd(realGantt.getTask('real-period-b').start_date), '2026-07-11', 'real dhtmlx standalone rejection restores the committed business date immediately')
assert.equal(realStoreCallbackCalls, 0, 'editable standalone rejection never invokes the store callback')
assert.equal(runRealDragLifecycle('real-period-b', '2026-07-10', '2026-07-20'), false, 'real dhtmlx rejects a business-order conflict before task update')
assert.deepEqual([asYmd(realGantt.getTask('real-period-b').start_date), asYmd(realGantt.getTask('real-period-b').end_date)], ['2026-07-11', '2026-07-20'], 'real dhtmlx restores rejected business dates in getTask')
assert.equal(realValidationCalls, 1, 'a rejected business drag runs validation exactly once')
assert.equal(realStoreCallbackCalls, 0, 'a rejected business drag never invokes the store callback')
assert.equal(realStoreTasks.find(task => task.id === 'real-period-b')?.planStartDate, '2026-07-11', 'rejected real lifecycle drag leaves store input unchanged')
assert.equal(runRealDragLifecycle('real-period-b', '2026-07-12', '2026-07-22'), true, 'real dhtmlx accepts a valid business drag')
assert.deepEqual([asYmd(realGantt.getTask('real-period-b').start_date), asYmd(realGantt.getTask('real-period-b').end_date)], ['2026-07-12', '2026-07-22'], 'real dhtmlx keeps accepted business dates')
assert.equal(realValidationCalls, 2, 'a legal real lifecycle drag runs validation exactly once')
assert.equal(realStoreCallbackCalls, 1, 'a legal real lifecycle drag invokes the store callback exactly once')
detachRealLifecycle()
realGantt.clearAll()

realGantt.parse({ data: [{ id: 'technical-real', text: '技术子项目', type: 'task', readonly: false, start_date: new Date('2026-08-01T00:00:00Z'), end_date: new Date('2026-08-10T00:00:00Z') }] })
const realTechnicalController = ganttRules.createPlanGanttInteractionController({ readOnly: false, getOnTaskDateChange: () => () => true, formatDate: asYmd, updateTask: task => realGantt.updateTask(task.id), refreshTask: task => realGantt.refreshTask(task.id) })
const detachRealTechnicalLifecycle = ganttRules.attachPlanGanttInteractionLifecycle(realGantt, realTechnicalController)
const realTechnicalTask = realGantt.getTask('technical-real')
realTechnicalTask.start_date = new Date('2026-08-02T00:00:00Z')
assert.equal(realGantt.callEvent('onBeforeTaskUpdate', ['technical-real', realTechnicalTask]), true, 'real technical gantt keeps default standalone update semantics')
assert.equal(asYmd(realGantt.getTask('technical-real').start_date), '2026-08-02', 'real technical gantt keeps the default accepted update value')
assert.equal(realGantt.callEvent('onBeforeLightbox', ['technical-real']), true, 'real technical gantt keeps default lightbox semantics')
detachRealTechnicalLifecycle()
realGantt.clearAll()
...
assert.match(projectSpaceSource, /applyPlanGanttDateChangeResult/, 'Gantt candidates use the shared validated result before they are written')
assert.match(projectSpaceSource, /message\.error\(result\.message\)/, 'Gantt rejection surfaces the shared exact validation message')
assert.match(projectSpaceSource, /allowLightbox=\{false\}/, 'governed level-one gantt disables the bypass-prone lightbox without changing technical gantts')
assert.match(projectSpaceSource, /allowStandaloneUpdate=\{false\}/, 'governed level-one gantt only accepts updates from its validated drag path')
