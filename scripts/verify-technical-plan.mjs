...
assert.equal(instanceStore.getState().plansByKey['9:subproject:IPM-AI-001'].versions[1].tasks[0].actualStartDate, '2026-08-07', 'draft actual start remains editable')
assert.equal(instanceStore.getState().plansByKey['9:subproject:IPM-AI-001'].versions[1].tasks[0].actualEndDate, '2026-08-08', 'draft actual completion remains editable')
assert.equal(instanceStore.getState().plansByKey['9:subproject:IPM-AI-001'].versions[0].tasks[0].actualStartDate, childV1.tasks[0].actualStartDate, 'draft writes never mutate the paired published actual start')
assert.equal(instanceStore.getState().plansByKey['9:subproject:IPM-AI-001'].versions[0].tasks[0].actualEndDate, childV1.tasks[0].actualEndDate, 'draft writes never mutate the paired published actual completion')
...
const beforePublishedWrite = instanceStore.getState().plansByKey['9:subproject:IPM-AI-001']
const publishedV2 = beforePublishedWrite.versions.find(version => version.versionNo === 'V2')
...
assert.deepEqual(instanceStore.updateCurrentTasks(childPlanScope, publishedWrite, 1), { ok: false, reason: 'historical-published' }, 'even the latest published version rejects every task write')
const afterPublishedWrite = instanceStore.getState().plansByKey['9:subproject:IPM-AI-001']
assert.deepEqual(afterPublishedWrite.versions, beforePublishedWrite.versions, 'a rejected published write cannot mutate either the published snapshot or paired draft')
