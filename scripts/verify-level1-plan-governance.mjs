...
assert.equal((projectSpaceSource.match(/resolveLevel1HorizontalVersionCells\(/g) || []).length >= 3, true, 'horizontal UI and export resolve every version/actual cell from that projection instead of a merged header row')
assert.doesNotMatch(projectSpaceSource, /vProjection\.rows\.find\([^\n]+\)\s*\|\|\s*milestone/, 'a historical horizontal cell never falls back to another version header object')
