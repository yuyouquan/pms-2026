...
check('creation persists simulated recipient/message snapshots; edits and failures never resend', () => {
  for (const [index, scope] of scopes.entries()) {
    ...
    assert.equal(notice.subject, `${entry.after.name}--请补全项目信息`)
    assert.ok(!notice.body.includes(entry.after.name))
    assert.ok(!notice.body.includes('通知对象'))
    const link = new URL(notice.actionUrl, 'https://pms-transsion.vercel.app')
    assert.equal(link.searchParams.get('projectId'), result.projectId)
    assert.equal(link.searchParams.get('action'), 'edit-basic-info')
