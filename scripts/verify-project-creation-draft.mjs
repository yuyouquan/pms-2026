...
assertOrdered(retryHydrationBlock, [
  "draftReadStatusRef.current !== 'failed'",
  'setDraftHydrationAttempt',
], 'retry must remain failed until a new hydration attempt starts')
assert.match(modalSource, /draftHydrationAttempt[\s\S]*startCreateDraftSession\(draftOwnerId \|\| ''\)/)
