...
assert.match(redesignBrowser, /const waitForControlledPopupCandidates = async \(page, input, label\)/, 'browser waits for non-empty candidates owned by the current target combobox')
assert.match(redesignBrowser, /const selectOption = async \(page, input, text/, 'option selection re-reads the current target combobox popup instead of retaining a popup id snapshot')
assert.match(redesignBrowser, /const selectedOption = await page\.waitForFunction\(\(targetInput, value, useContains\)/, 'option lookup and click happen atomically against the current controlled popup')
assert.match(redesignBrowser, /await selectedOption\.dispose\(\)/, 'controlled option selection completes its atomic browser evaluation')
assert.doesNotMatch(redesignBrowser, /const candidateState = await page\.evaluate/, 'IPM selection does not reject a transient empty candidate snapshot before the bounded wait')
...
assert.match(redesignBrowser, /const isColdCompileTransient = message =>/, 'prewarm retries are explicitly classified as cold compile or chunk transients')
...
const addProjectModal = readSource(root, 'src/components/workspace/AddProjectModal.tsx')
const themeProvider = readSource(root, 'src/components/shared/PmsThemeProvider.tsx')
...
assert.doesNotMatch(modal, /Modal\.confirm\(/, 'project modal must not emit static-context modal warnings')
...
assert.match(technicalPlan, /dataSource=\{tdtTreeRows\}[\s\S]{0,500}rowClassName=\{row => row\.parentId \? 'pms-level1-row-level-1' : 'pms-level1-row-level-0'\}/, 'TDT vertical plan renders phase parents and milestone children')
assert.match(technicalPlan, /dataSource=\{visibleTasks\}[\s\S]{0,240}rowClassName=\{\(\) => ['"]technical-plan-flat-row['"]\}/, 'technical subproject vertical plan remains a single-level activity table')
const projectListMatrixSource = readSource(root, 'src/lib/projectListMatrix.ts')
assert.match(projectListMatrixSource, /PROJECT_LIST_FIXED_COLUMN_KEYS[\s\S]{0,160}machine:\s*\[\]/, 'current project-list matrix explicitly keeps machine columns unpinned')
