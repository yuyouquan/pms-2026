...
assert.match(technicalInformationView, /ProjectInformationFrame/, 'technical information reuses the whole-machine information frame')
assert.match(technicalInformationView, /TechnicalPlanSummary/, 'technical information renders the technical plan summary in the shared frame')
assert.match(technicalInformationView, /activeTab\.kind\s*===\s*['"]subproject['"]/, 'only a subproject tab renders technical basic information')
assert.match(technicalInformationView, /CollapsibleInformationSection/, 'technical information uses collapsible sections for extended details')
assert.match(technicalInformationView, /CollapsibleInformationSection[\s\S]{0,180}(团队|交付物)|(团队|交付物)[\s\S]{0,180}CollapsibleInformationSection/, 'technical team and deliverables are inside collapsible information sections')
assert.doesNotMatch(technicalInformationView, /activeTab\.kind\s*===\s*['"]tdt['"][\s\S]{0,240}(TechnicalProjectBasicInfo|基础信息)/, 'TDT tabs do not render technical basic information')
...
assert.match(projectSpace, /<TechnicalProjectInformationView/, 'project space mounts the shared-frame technical information view')
