...
assert.match(projectList, /const \{ message: messageApi \} = App\.useApp\(\)/, 'access feedback consumes the mounted application context')
assert.match(projectList, /const showProjectAccessDenied = \(\) => messageApi\.warning/, 'access denial provides user feedback')
