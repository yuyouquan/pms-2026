...
const projectConfigurationSource = fs.readFileSync(path.join(root, 'src/components/project-management/ProjectConfiguration.tsx'), 'utf8')
...
assert.ok(technicalTypeRowIndex < 0 || technicalTypeRowIndex < filterSummaryIndex, 'technical project type precedes active filters')
assert.match(source, /aria-label="项目列表快捷筛选"/, 'quick filters render below project status and technical type')
assert.match(source, /toolbarHost=\{projectListTableToolbarHost\}/, 'summary controls render into the category action rail')
assert.doesNotMatch(source, /AddProjectModal|addProjectOpen|setAddProjectOpen|aria-label="新增项目"/, 'project view does not retain the retired standalone creation flow')
assert.match(projectConfigurationSource, /canCreate \? <Button[^\n]*icon=\{<PlusOutlined \/>\}[^\n]*setNewProjectOpen\(true\)[^\n]*>新项目<\/Button>/, 'project creation lives in the permission-gated configuration action')
assert.doesNotMatch(source, /projectListView === 'card'[^\n]*workbenchListState\.showSecondaryCategory/)
assert.match(source, /const statusEnumType = getProjectStatusEnumType\(projectTypeFilter\)/, 'project-list status options resolve the current category enum type')
assert.doesNotMatch(source, /const statusHistory\s*=/, 'retired historical statuses are not reintroduced into quick filters')
assert.match(source, /useSingleEnumOptions\(\s*statusEnumType,\s*undefined,\s*projectTypeFilter !== 'all',\s*'filter',?\s*\)/, 'status quick filters use the current category catalog including historical filter values')
assert.match(source, /useEnumHydration\(projectTypeFilter !== ['"]all['"]\)/, 'status controls wait for category enum hydration')
assert.match(source, /projectTypeFilter === ['"]all['"] \|\| !statusHasHydrated \|\| statusHydrationError[\s\S]{0,100}: configuredStatusOptions/, 'status quick filters expose hydrated configured options only when the category is ready')
assert.doesNotMatch(source, /TOS_PROJECT_LIST_STATUS_OPTIONS/, 'project list no longer depends on the removed tOS-only status constant')
assert.match(source, /showQuickFilters/)
assert.match(source, /quickFilterHost=\{projectListQuickFilterHost\}/)
...
const machineSeriesToggle = summarySource.match(/<button[\s\S]{0,500}pms-machine-series-toggle[\s\S]{0,1200}<\/button>/)?.[0] || ''
assert.ok(machineSeriesToggle, 'machine product-series collapse control remains rendered')
assert.doesNotMatch(machineSeriesToggle, /<small>[\s\S]*?个项目<\/small>/, 'machine product-series label does not repeat its project count')
...
assert.match(summarySource, /快捷筛选-项目名称/, 'project list exposes the requested project-name quick filter')
...
assert.match(matrixSource, /listField\('productSeries', '产品系列', true, 148\)/, 'machine product-series width is 148px')
assert.match(matrixSource, /listField\('projectName', '项目名称', true, 200\)/, 'machine project-name width is 200px')
...
assert.match(styles, /\.pms-project-list-filter-grid,[\s\S]{0,100}height:\s*auto/s, 'category filter surface expands for the added quick-filter row')
assert.match(styles, /\.pms-project-list-quick-filter-row/, 'quick-filter row uses the shared project-list layout')
...
assert.match(styles, /\.pms-project-summary-table \.ant-table-cell\.pms-project-name-cell\s*\{[^}]*padding-inline:\s*12px\s*!important/s, 'project-name header and body use identical horizontal padding')
