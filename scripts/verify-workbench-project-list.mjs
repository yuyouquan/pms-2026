...
assert.match(source, /const statusHistory = useMemo\(\(\) => \[\.\.\.new Set\(projects[\s\S]{0,420}\.map\(project => project\.status\)[\s\S]{0,80}\.filter\(Boolean\)\)\]/, 'status hydration preserves statuses already present in the filtered project history')
assert.match(source, /useSingleEnumOptions\(\s*statusEnumType,\s*statusHistory,\s*projectTypeFilter !== ['"]all['"],?\s*\)/, 'status options use the shared enum hook with history fallback and category-scoped loading')
