...
const { buildCumulativeLabor } = get('cumulativeLaborData')
...
const cumulative = buildCumulativeLabor(dataset, { ...filter, year: '2028', startDate: '2028-01-01', endDate: '2028-12-31' }, '2027-01-04')
close(cumulative.labor, .22, 'cumulative ignores top date and year, includes today, excludes future')
assert.equal(cumulative.rows.length, 1)
assert.equal(cumulative.rows[0].peopleCount, 1)
close(cumulative.rows[0].personDays, 5, 'source person-days unchanged')
close(buildCumulativeLabor(dataset, {}, '2027-01-05', '2027-01-01').labor, .22, 'project start boundary excludes older records')
close(buildCumulativeLabor(dataset, { primary: '硬件' }, '2027-01-05').labor, .1, 'department selection affects cumulative')
assert.equal(buildCumulativeLabor(dataset, {}, '2026-11-01').rows.length, 0, 'future-only mock produces no life-to-date labor')
assert.equal(buildCumulativeLabor(undefined, {}, '2027-01-05'), undefined)
...
get('exportResourceBusinessDashboard').exportResourceBusinessDashboard('测试', [full, planned, undefined], actual, filter, 'cost', 'stage', [definition, capability, undefined, definition], cumulative)
...
close(sheet('累至今日的人力投入').rows[0].labor, .22, 'cumulative export includes department totals')
assert.equal(sheet('累至今日的人力投入').rows[0].range, '2026-12-01～2027-01-04', 'cumulative export does not inherit top range')
