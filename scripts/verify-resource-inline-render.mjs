...
const store = { projects: [owner], monthlyInvestments: [], createResourceVersion: (_project, _budget, _scope, options) => { const id = String(++serial); owner.versions.push({ ...version(id), versionNumber: `V${options.versionNumber}` }); return id } }
...
function validate(tree) {
  ...
  assert.equal(all.find(node => node.type === 'Detail').props.version.id, states[2] ?? '1')
  return all
}
let all = validate(render())
for (let i = 0; i < 3; i++) {
  all.find(node => node.type === 'Button' && node.props.children === '新建版本').props.onClick()
  elements(render()).find(node => node.type === 'CreateDialog').props.onCreate({ versionNumber: `7.${i}` })
  all = validate(render())
...
assert.ok(activeNodes.some(node => node.type === 'FlagFilled' && node.props['aria-label'] === '正式版本'))
...
assert.equal(inactiveNodes.some(node => node.type === 'FlagFilled'),false,'inactive versions have no activation marker')
...
assert.ok(activeNodes.some(node => node.type === 'Tooltip' && node.props.title === '正式版本'))
