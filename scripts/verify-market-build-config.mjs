...
assert.match(marketEditorSource, /useSingleEnumOptions\('build-option', buildOptionHistory, open\)/, 'build options come from the configured enum with historical selection compatibility')
assert.match(marketEditorSource, /useSingleEnumOptions\('build-market', buildMarketHistory, open\)/, 'build markets come from the configured enum with historical selection compatibility')
assert.match(marketEditorSource, /useEnumHydration\(open\)/, 'the market editor tracks enum loading and failure state')
assert.doesNotMatch(marketEditorSource, /mockSpugBuildOptionsProvider|loadSpugBuildOptions/, 'configured enum values replace the obsolete SPUG mock provider')
...
assert.match(marketEditorSource, /const handleSave =[\s\S]*useEnumStore\.getState\(\)[\s\S]*!enumState\.hasHydrated \|\| enumState\.hydrationError[\s\S]*firstRequiredIssue[\s\S]*unsupportedIssues\[0\][\s\S]*onSave\(\)/, 'market save rechecks live enum readiness before required and unsupported selection guards')
assert.match(marketEditorSource, /saveDisabled=\{[\s\S]{0,240}!enumReady[\s\S]{0,240}unsupportedIssues\.length/, 'market save is disabled while enum data is unavailable or any selection is unsupported')
assert.match(marketEditorSource, /onClick=\{\(\) => void retryHydration\(\)\}>重试</, 'the market editor exposes an enum loading retry action')
...
assert.match(projectSpaceSource, /const currentPlanScopeUnavailable = projectPlanLevel === 'level1' && machineMarketPlanUnavailable/, 'the scope empty state is limited to level-one plans')
assert.match(projectSpaceSource, /!usesSharedPlanWorkspace && planWorkspacePrimaryScopeTabs\}[\s\S]{0,100}\{currentPlanScopeUnavailable/, 'scope navigation stays visible above the empty-state content')
