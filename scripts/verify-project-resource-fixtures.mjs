...
for(let i=0;i<4;i++)check(`${categories[i]}: published own dates, annual history, unique counts, manual monthly split and source references`,()=>{
 ...
 for(const v of f.versions){
  ...
  if(cat==='tos'||cat==='machine') {
   const manualKeys=cat==='tos'?['marketIteration','maintenanceEnd']:['productLaunch','lifecycleEnd']
...
check('fresh multi-owner, partial milestones and cancelled budget retain distinct interaction states',()=>{
 ...
 const technical=registry.getState().projects.find(p=>p.id==='mock-budget-technical-unbound')
 assert.deepEqual(technical.responsiblePersons,['演示用户01','演示用户02'])
 assert.deepEqual(buildProjectInfoValues(technical,['technicalLead']).technicalLead,technical.responsiblePersons)
 const tos=record(stores[1],'mock-budget-tos-unbound')
 assert.ok(tos.versions[0].milestones.maintenanceEnd)
 assert.equal(tos.versions[1].milestones.maintenanceEnd,null)
 assert.ok(tos.versions[1].milestones.planningKO)
 const cancelled=record(stores[0],'mock-budget-machine-cancelled')
 assert.equal(cancelled.status,'cancelled');assert.equal(cancelled.versions.length,1)
 assert.equal(access.canAccessHrProject(cancelled,false),true)
 const history=structuredClone(cancelled.versions)
 assert.equal(rules.canCreateHrVersion(cancelled,'annual'),false)
