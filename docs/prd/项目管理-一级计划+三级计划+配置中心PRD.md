...
![一级计划版本与修订](./assets/project-management-plan-prd/12-level1-version-selector-latest-dev.png)
...
5. 一级计划不提供克隆功能；已发布版本仅允许查看、对比或创建修订。
6. 每次创建修订，任务结构必须读取配置中心最新已发布的一级计划模板，不复制旧项目版本的结构。
7. 对用户已填写的任务，按去除首尾空格后的“任务名”匹配旧版本数据，不以大阶段、层级或稳定ID作为回填条件。任务被模板移动到其他大阶段后，只要任务名相同仍应回填。
8. 命中同名任务时，回填计划开始、计划完成、预估工期、实际开始、实际完成、实际工期六个时间字段；最新模板新增且历史不存在同名任务的节点保持空值。
9. 用户自定义新增的MR/tOS业务节点在其所属大阶段仍存在时自动保留；最新模板删除该大阶段后，其下自定义节点一并移除。
10. 创建修订成功后系统必须直接切换到该修订版本并进入可编辑状态。
11. 同行计划开始时间不得晚于计划结束时间；子任务日期不得超出父任务日期范围。
12. 发现错误时单元格标红并显示错误提示；存在错误时禁止发布并滚动到首个错误。

### 7.3 横版版本发布日期与基础信息修订行

![一级计划横版新增节点历史空值](./assets/project-management-plan-prd/13-level1-horizontal-new-node-latest-dev.png)
...
6. 横版表头以配置中心最新已发布模板为准，并在对应大阶段下合并展示仍被保留的自定义MR/tOS业务节点。
7. 当最新模板或当前修订新增节点时，早期已发布版本若不存在同名任务，对应单元格显示`-`，不得借用其他任务或修订中的时间。
