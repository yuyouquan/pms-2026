...
- `roadmap:view`：进入项目路标、查看两种视图、筛选、排序、列设置、查看修改记录。
- `roadmap:edit`：创建、编辑、删除待规划项目，维护 tOS 版本和版本目标。
...
9. 维护 tOS 版本，验证降序、重名校验、引用保护和改名级联。
10. 新增、修改、清空版本目标，验证两种视图显示。
11. 表单视图切换单个版本，验证全部业务列排序、筛选和列设置。
12. 版本演进视图验证左旧右新、进入定位最右侧、共享纵向滚动、版本/目标吸顶、新品/老品对齐和品牌顺序。
13. 验证修改记录的来源、动作、日期筛选、字段顺序和分页。
14. 验证 `roadmap:view`、`roadmap:edit` 和管理组权限。
15. 在减少动态效果设置下重复核心视图切换。
