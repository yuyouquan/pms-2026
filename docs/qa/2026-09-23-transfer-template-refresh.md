...
- 首次线上回查发现 tOS 录入页在加载后触发 React #185。回查并非返回按钮错误。
- 用独立本地测试页面模拟项目 2 缺失角色配置，稳定复现 Maximum update depth exceeded。空角色数组每次渲染创建新引用，导致计划快照初始化 effect 重复执行；无变化的快照 setter 仍通知整个项目空间，形成循环。
- 修复：缓存缺失角色的空数组；快照更新前比较引用，无变化时保留 Zustand 状态引用。
- 新增 `verify-plan-snapshot-stability.mjs`：先验证 60 次无变化初始化错误地通知 60 次，修复后通知 0 次；真实变更仍只通知一次。
- 相同缺失角色场景下，tOS 录入页可稳定读取 V-01、展示 23 条 SPM 资料并返回工作台，修复后无新增浏览器错误。
