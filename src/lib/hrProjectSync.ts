...
import { HR_BUDGET_TYPES, getActiveHrVersion, getMachineProjectYear, isLatestHrVersion, normalizeHrVersionSequence, type HrVersionIdentity } from '@/lib/hrVersionRules'
...
export function synchronizeHrProjects<T extends SyncProject>(
  ...
  return projects.map(input => {
    ...
    for (const budgetType of HR_BUDGET_TYPES) updated[budgetType === 'annual' ? 'annualBudget' : budgetType] = getActiveHrVersion(versions, budgetType)?.estimatedInvestment ?? 0
    if (category === 'machine') {
