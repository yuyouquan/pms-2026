...
export function refreshTransferMockState(state: TransferState): Partial<TransferState> | null {
  if (state.tmMockTemplateRevision === TRANSFER_TEMPLATE_REVISION) return null
  ...
  for (const kind of TRANSFER_PROJECT_TYPES) {
    if (transferMockFingerprint(state.tmTeamConfigs[kind]) === transferMockFingerprint(getTransferRoleConfig(kind))
      && transferMockFingerprint(state.tmTemplateVersions[kind]) === templateBaselines[kind]) tmTemplateVersions[kind] = current[kind]
  }
  ...
  return {
    tmMockTemplateRevision: TRANSFER_TEMPLATE_REVISION, tmTemplateVersions, transferApplications: nextApplications,
