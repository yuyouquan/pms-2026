...
import type { TechMonthlyInvestment, BudgetType } from '@/types/hrTechnical'
...
interface MonthlyInvestmentRow extends TechMonthlyInvestment {
  projectName: string
}
...
export default function MonthlyInvestmentTab() {
  const monthlyInvestments = useHrTechnicalStore((s) => s.monthlyInvestments)
  ...
  // ── 筛选状态（多选，空数组=不筛选） ──────────────────────
  const ALL_BUDGET_TYPES: BudgetType[] = TECH_BUDGET_TYPES.map((t) => t.value)
  ...
  const budgetTypeOptions = useMemo(
    () => TECH_BUDGET_TYPES.map((t) => ({ label: t.label, value: t.value })),
    [],
  )
  ...
  const projectMap = useMemo(() => {
    ...
    projects.forEach((project) => map.set(project.id, project.tdtName))
    return map
  }, [projects])
  ...
  const columns = useMemo<ColumnsType<MonthlyInvestmentRow>>(() => {
    const baseColumns: ColumnsType<MonthlyInvestmentRow> = [
      ...
      },
      {
        ...
        key: 'budgetType',
        width: 100,
        render: (value: TechMonthlyInvestment['budgetType']) =>
          TECH_BUDGET_TYPE_LABELS[value] ?? value,
      },
      {
        title: '版本号',
  ...
  const handleExport = () => {
    const exportColumns: ExportColumn[] = [
      { key: 'projectName', title: 'TDT项目名称', width: 20 },
      ...
      { key: 'secondaryDepartment', title: '二级部门', width: 12 },
      {
        ...
        title: '预算类型',
        width: 10,
        formatter: (value) => TECH_BUDGET_TYPE_LABELS[value as BudgetType] ?? String(value),
      },
    ...
    exportSheet(
      dataSource,
      exportColumns,
      `技术项目月度预估投入_${exportTimestamp()}.xlsx`,
      '月度预估投入',
    )
  }

  return (
    <div className="pms-hr-tech-monthly-investment-tab">
      <Card
      ...
      <style jsx global>{`
        .pms-hr-tech-monthly-investment-tab .pms-table {
          font-variant-numeric: tabular-nums;
        }
        /* 合计行：品牌色背景突出显示 */
        .pms-hr-tech-monthly-investment-tab .pms-table .pms-summary-row > td {
          background: var(--pms-brand-surface) !important;
          color: var(--pms-brand-strong);
          font-weight: 600;
          border-top: 2px solid var(--pms-brand-border);
        }
        .pms-hr-tech-monthly-investment-tab .pms-table .pms-summary-row:hover > td {
          background: var(--pms-brand-surface) !important;
        }
        /* 已锁定版本：绿色背景 */
        .pms-hr-tech-monthly-investment-tab .pms-table .pms-row-locked > td {
          background: #f0f9eb !important;
        }
        .pms-hr-tech-monthly-investment-tab .pms-table .pms-row-locked:hover > td {
          background: #e8f5e0 !important;
        }
        /* 未锁定版本：灰色背景 */
        .pms-hr-tech-monthly-investment-tab .pms-table .pms-row-unlocked > td {
          background: #f5f5f5 !important;
        }
        .pms-hr-tech-monthly-investment-tab .pms-table .pms-row-unlocked:hover > td {
