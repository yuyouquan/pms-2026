...
function LongText({ value }: { value?: string }) {
  ...
  return <Popover placement="topLeft" trigger={['hover', 'click', 'focus']} content={content}><span tabIndex={0} style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', display: '-webkit-box', WebkitBoxOrient: 'vertical', WebkitLineClamp: 3, overflow: 'hidden', cursor: 'pointer' }}>{value}</span></Popover>
}
function Personnel({ name, delegates, side }: { name: string; delegates?: string[]; side: '录入' | '审核' }) {
  return <Space orientation="vertical" size={3}><span>{name || '-'} {Boolean(delegates?.length) && <Tag color="purple">已委派</Tag>}</span>{delegates?.map(id => <Tag key={id} color="blue">{side}委派→{personName(id)}</Tag>)}</Space>
}
...
function PageHeading({ props, app, title, children }: { props: TransferModuleProps; app: TransferApplication; title: string; children?: ReactNode }) {
  useEffect(() => { document.getElementById('basic-info-scroll-container')?.scrollTo({ top: 0 }) }, [app.id, title])
  return <><div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}><div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0, flex: '1 1 320px' }}><Button icon={<ArrowLeftOutlined />} style={{ flexShrink: 0 }} onClick={() => props.setTransferView(null)}>返回</Button><span style={{ fontSize: 19, fontWeight: 600, minWidth: 0, overflowWrap: 'anywhere' }}>{app.projectName} · {title}</span></div>{children}</div><Card id="transfer-pipeline" className="pms-solid-surface" style={{ marginBottom: 16 }}><TransferPipelineProgress pipeline={app.pipeline} /></Card></>
}
...
function materialColumns(kind: 'checklist' | 'review', options: { search?: ReturnType<typeof useTransferColumnSearch>; onAiDetail?: (item: TransferItem) => void; onRejected?: (item: TransferItem) => void; aiFailuresOnly?: boolean; showAiRule?: boolean } = {}): ColumnsType<TransferItem> {
  ...
  return [
    ...
    ...(kind === 'checklist' ? [{ title: '类型', dataIndex: 'type', width: 100 }, { title: '检查项目名称', dataIndex: 'checkItem', width: 260, render: (value: string) => <LongText value={value} />, ...search?.('checkItem', '检查项目名称') }] : [{ title: '标准', dataIndex: 'standard', width: 150, render: (value: string) => <LongText value={value} /> }, { title: '评审要素', dataIndex: 'description', width: 240, render: (value: string) => <LongText value={value} />, ...search?.('description', '评审要素') }, { title: '模板备注', dataIndex: 'remark', width: 160, render: (value: string) => <LongText value={value} /> }]),
    { title: '责任角色', dataIndex: 'responsibleRole', width: 110 },
    { title: '资料录入人员', width: 180, render: (_: unknown, item: TransferItem) => <Personnel name={item.entryPerson} delegates={item.delegatedTo} side="录入" /> },
    { title: '审核人员', width: 180, render: (_: unknown, item: TransferItem) => <Personnel name={item.reviewPerson} delegates={item.reviewDelegatedTo} side="审核" /> },
    ...(options.showAiRule ? [{ title: '智能检查规则', dataIndex: 'aiCheckRule', width: 200, render: (value: string) => <LongText value={value} /> }] : []),
    { title: '交付件/录入内容', width: 220, render: (_: unknown, item: TransferItem) => <EntryContent item={item} /> },
    { title: '录入状态', dataIndex: 'entryStatus', width: 100, render: tag, ...(search ? statusFilter('entryStatus', [{ text: '未录入', value: 'not_entered' }, { text: '暂存', value: 'draft' }, { text: '已录入', value: 'entered' }]) : {}) },
    { title: 'AI检查', dataIndex: 'aiCheckStatus', width: 100, ...(search ? statusFilter('aiCheckStatus', Object.entries(AI_STATUS).map(([value, status]) => ({ value, text: status.label }))) : {}), render: (value: TransferItem['aiCheckStatus'], item: TransferItem) => {
    ...
    { title: '维护审核状态', dataIndex: 'reviewStatus', width: 115, render: (value: string, item: TransferItem) => options.onRejected && value === 'rejected' ? <Button type="text" size="small" aria-label="查看审核意见" style={{ padding: 0 }} onClick={() => options.onRejected?.(item)}>{tag(value)}</Button> : tag(value) },
    { title: '审核备注', dataIndex: 'reviewRemark', width: 190, render: (value: string) => <LongText value={value} /> },
    { title: '评审意见', dataIndex: 'reviewComment', width: 190, render: (value: string) => <LongText value={value} /> },
  ]
}
...
export function TransferWorkbench(props: TransferModuleProps & { embedded?: boolean }) {
  ...
  const table = <><Table className="pms-table" dataSource={apps} rowKey="id" size="small" pagination={false} scroll={{ x: 1100 }} columns={[
  ...
  ]} /><Modal className="pms-modal" title="关闭转维流水线" open={Boolean(closingId)} onCancel={() => setClosingId(null)} okText="确认关闭" okButtonProps={{ danger: true }} onOk={() => {
...
export function TransferApply(props: TransferModuleProps) {
  ...
  return <Modal className="pms-modal" title={<Space><SwapOutlined />{source ? '重新发起转维' : '申请转维'}</Space>} open width={940} onCancel={cancel} onOk={submit} okText="提交申请" confirmLoading={submitting} destroyOnHidden>
...
function TaskPanels({ props, app, editLegacy = false, resolveLegacy = false }: { props: TransferModuleProps; app: TransferApplication; editLegacy?: boolean; resolveLegacy?: boolean }) {
  ...
  return <><Card id="transfer-block" title="Block任务列表" style={{ marginBottom: 16 }}><Table<BlockTask> className="pms-table" rowKey="id" size="small" pagination={false} scroll={{ x: 1000 }} dataSource={props.tmBlockTasks.filter(task => task.applicationId === app.id)} columns={taskColumns(true)} /></Card>
    ...
    <Modal className="pms-modal" title="添加遗留任务" open={adding} width={720} onCancel={() => setAdding(false)} okText="保存遗留任务" onOk={() => {
      const current = freshApp(props, app.id)
...
function MaterialPanels({ props, app }: { props: TransferModuleProps; app: TransferApplication }) {
  ...
  return <><Card id="transfer-checklist" title="转维CheckList" style={{ marginBottom: 16 }}><Table<TransferItem> className="pms-table" rowKey="id" size="small" pagination={false} scroll={{ x: 2000 }} dataSource={props.tmChecklistItems.filter(item => item.applicationId === app.id)} columns={materialColumns('checklist', options)} /></Card>{getTransferProjectType(app.projectType || props.selectedProject) !== 'tOS版本项目' && <Card id="transfer-review" title="转维要素评审列表" style={{ marginBottom: 16 }}><Table<TransferItem> className="pms-table" rowKey="id" size="small" pagination={false} scroll={{ x: 2200 }} dataSource={props.tmReviewElements.filter(item => item.applicationId === app.id)} columns={materialColumns('review', options)} /></Card>}<Modal className="pms-modal" title={result?.title} open={Boolean(result)} onCancel={() => setResult(null)} footer={<Button onClick={() => setResult(null)}>关闭</Button>}><div style={{ maxHeight: '60vh', overflow: 'auto', whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{result?.content}</div></Modal></>
}
function TransferProjectPanels({ props, app }: { props: TransferModuleProps; app: TransferApplication }) {
  return <><Card id="transfer-info" title="项目信息" style={{ marginBottom: 16 }}><Descriptions column={3} size="small"><Descriptions.Item label="项目名称">{app.projectName}</Descriptions.Item><Descriptions.Item label="项目编号">{props.selectedProject?.projectCode || app.projectId}</Descriptions.Item><Descriptions.Item label="项目类型">{getTransferProjectType(app.projectType || props.selectedProject)}</Descriptions.Item><Descriptions.Item label="项目负责人">{app.applicant}</Descriptions.Item><Descriptions.Item label="维护SPM">{getTransferMember(app.team.maintenance, app.finalReviewRole || 'SPM', app.teamConfig)?.name || '-'}</Descriptions.Item><Descriptions.Item label="转维启动时间">{app.createdAt.slice(0, 10)}</Descriptions.Item><Descriptions.Item label="计划评审日期">{app.plannedReviewDate}</Descriptions.Item><Descriptions.Item label="项目状态">{tag(app.status)}</Descriptions.Item><Descriptions.Item label="备注" span={3}><LongText value={app.remark} /></Descriptions.Item></Descriptions></Card>
    <Row id="transfer-team" gutter={16} style={{ marginBottom: 16 }}>{(['research', 'maintenance'] as const).map(side => <Col span={12} key={side}><Card size="small" title={side === 'research' ? '在研团队' : '维护团队'}><Space wrap>{app.team[side].map(member => <Space key={`${member.id}-${member.role}`} style={{ padding: 8, border: '1px solid var(--pms-border-subtle, #e5e7eb)', borderRadius: 8 }}><Avatar size={28} style={{ background: ROLE_COLORS[member.role] || 'var(--pms-brand)' }}>{member.name.slice(-1)}</Avatar><div>{member.name}<div style={{ fontSize: 12, color: '#6b7280' }}>{member.role} · {member.department || '未配置部门'}</div></div></Space>)}</Space></Card></Col>)}</Row></>
}
...
function TransferItems(props: TransferModuleProps & { side: 'entry' | 'review' }) {
  ...
  const columns = (kind: 'checklist' | 'review'): ColumnsType<TransferItem> => [...materialColumns(kind, { search: columnSearch, showAiRule: side === 'entry', onAiDetail: side === 'entry' ? item => setAiDetailId(item.id) : undefined }), { title: '操作', width: side === 'entry' ? 150 : 200, fixed: 'right' as const, render: (_: unknown, item: TransferItem) => <Space size={0} wrap>
    ...
    {canDelegateTransferItem(app, item, props.currentUser, props.selectedProject, side) && <Button type="link" size="small" onClick={() => open('delegate', [item])}>委派{side === 'review' ? '审核' : ''}</Button>}
  </Space> }]
  ...
  return <div style={{ maxWidth: 1600, margin: '0 auto' }}><PageHeading props={props} app={app} title={side === 'entry' ? '资料录入与AI检查' : '维护审核'} />
    {effectiveRole && <Card style={{ marginBottom: 16 }}><Space wrap style={{ width: '100%', justifyContent: 'space-between' }}><Segmented value={effectiveRole} options={roles.map(value => ({ value, label: `${value}角色` }))} onChange={value => setRole(String(value))} /><Space>
      {side === 'entry' && ownsRole && <Tooltip title={submitHint}><Button type="primary" disabled={!canSubmit} onClick={() => open('submit', roleItems, effectiveRole)}>提交审核</Button></Tooltip>}
      {roleLegacyAllowed && <Button icon={<PlusOutlined />} onClick={() => open('role-legacy', roleItems, effectiveRole)}>追加遗留任务</Button>}
      {roleReviewAllowed && <><Button danger onClick={() => open('role-reject', roleItems, effectiveRole)}>角色审核不通过</Button><Button type="primary" onClick={() => open('role-pass', roleItems, effectiveRole)}>角色审核通过</Button></>}
    </Space></Space></Card>}
    {side === 'entry' && <Alert type="info" showIcon message={effectiveRole ? '确认录入后运行模拟AI检查；本角色资料全部录入并通过检查后，点击“提交审核”。' : '完成委派资料的录入并通过AI检查后，由对应角色负责人提交维护审核。'} style={{ marginBottom: 16 }} />}
