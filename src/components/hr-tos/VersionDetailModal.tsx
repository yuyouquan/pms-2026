...
export default function VersionDetailModal({
  ...
  const editingVersionId = useHrTosStore((s) => s.editingVersionId)
  ...
  const milestoneForm = useHrVersionMilestones('tos', project ?? undefined, version?.budgetType ?? 'annual', open, versionId ?? undefined, requestedReadOnly)
  const nonLabor = useNonLaborDraft(open, `${versionId ?? ''}:${requestedReadOnly}`,  version?.nonLaborInvestment, 'tos', readOnly ? version?.milestones ?? {} : milestoneForm.values)
  ...
  const updateRow = (id: string, field: keyof TosDepartmentInvestment, value: string | number) => {
    setEditData((prev) =>
      prev.map((d) => {
        ...
        // 如果修改的是阶段预估投入字段，重新计算该行的 estimatedInvestment
  ...
  const columns: ColumnsType<TosDepartmentInvestment> = useMemo(() => {
    const base: ColumnsType<TosDepartmentInvestment> = [
      {
        title: '一级部门',
