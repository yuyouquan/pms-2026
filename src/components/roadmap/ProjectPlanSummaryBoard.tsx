...
const CATEGORY_ORDER = ['示例系列D', '示例系列A', '示例系列B', '示例系列E', 'tOS版本', '技术项目']

const CATEGORY_THEME: Record<string, { key: string; label?: string; color: string; bg: string; seriesBg: string; accent: string }> = {
  示例系列D: { key: 'camon', color: '#2563eb', bg: '#eff6ff', seriesBg: '#f8fbff', accent: '#3b82f6' },
  示例系列A: { key: 'note', label: '示例系列A', color: 'var(--pms-brand-strong)', bg: 'var(--pms-brand-surface)', seriesBg: 'color-mix(in srgb, var(--pms-brand) 4%, var(--pms-surface-solid))', accent: 'var(--pms-brand)' },
  示例系列B: { key: 'spark', color: '#059669', bg: '#ecfdf5', seriesBg: '#f4fff9', accent: '#10b981' },
  示例系列E: { key: 'pova', color: '#d97706', bg: '#fffbeb', seriesBg: '#fffdf2', accent: '#f59e0b' },
  ...
  for (const project of projects) {
    ...
    if (project.type === PROJECT_TYPE_TECH) {
      ...
      rows.push({
        ...
        status,
        spm: project.spm || project.leader || '-',
        department: getDepartmentByFirstSpm(project, '示例集成领域部'),
        milestones,
