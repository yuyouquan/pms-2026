...
export default function TosVersionMaintenanceModal({
  ...
  const getReferenceCounts = (versionId: string) => {
    const normalReferenceCount = normalRows.filter(row => row.firstSaleTosVersionId === versionId).length
    const plannedReferenceCount = plannedRows.filter(row => row.firstSaleTosVersionId === versionId).length
    return {
    ...
    >
      <Flex vertical gap={16}>
        ...
        <List
          ...
          renderItem={version => {
            const { normalReferenceCount, plannedReferenceCount, referenceCount } = getReferenceCounts(version.id)
                  ...
                  <Flex justify="space-between" align="center" gap={16} wrap>
                    ...
                    {canEdit ? (
                      <Flex gap={8} wrap>
                        ...
                        {referenceCount > 0 ? (
                          <Tooltip title={`已被 ${referenceCount} 个项目引用，无法删除`}>
                            <span>
