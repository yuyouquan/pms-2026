...
import { buildResourceMonthlyCostView, sumResourceCost, type ResourceMonthlyCostRow } from '@/components/project-resources/resourceMonthlyCost'
...
export default function ResourceVersionViews({ category, version, rows, readOnly, onSaveMonth }: {
  ...
  const amount = (value: number) => value.toFixed(cost ? 2 : 1)
  const range = hrNonLaborMonthRange(category, 'milestones' in version ? version.milestones : version, true)
  const expenseMonths = version.nonLaborInvestment ? nonLaborMonths(version.nonLaborInvestment) : []
  const complete = buildResourceMonthlyView(rows, version.id, range.startMonth ?? undefined, range.endMonth ?? undefined, 'all', expenseMonths)
  const selectedYear = complete.years.includes(year) ? year : 'all'
  const view = selectedYear === 'all' ? complete : buildResourceMonthlyView(rows, version.id, range.startMonth ?? undefined, range.endMonth ?? undefined, selectedYear, expenseMonths)
  const costs = buildResourceMonthlyCostView(complete, version.nonLaborInvestment, rate, version.estimatedInvestment)
  const totals = cost ? costs.totals : complete.totals
  const allocatedTotal = cost ? costs.allocatedTotal : complete.allocatedTotal
  const estimatedTotal = cost ? costs.estimatedTotal : version.estimatedInvestment
  const sumRow = (row: ResourceMonthlyRow, months?: string[]) => cost ? sumResourceCost(row.monthlyData, months) : sumMonthlyRow(row, months)
  ...
  const columns: ColumnsType<ResourceMonthlyCostRow> = [
    { title: '一级部门', dataIndex: 'primaryDepartment', width: 125, fixed: 'left', align: 'center', onCell: row => ({ colSpan: row.isNonLabor ? 2 : 1 }) },
    { title: '二级部门', dataIndex: 'secondaryDepartment', width: 125, fixed: 'left', align: 'center', onCell: row => ({ colSpan: row.isNonLabor ? 0 : 1 }) },
    ...
    ...visibleYears.map(value => ({ title: `${value}小计`, key: value, width: 110, align: 'center' as const,
      render: (_: unknown, row: ResourceMonthlyRow) => amount(sumRow(row, complete.months.filter(month => month.startsWith(`${value}-`)))) })),
    { title: '全周期合计', key: 'all', width: 110, align: 'center', render: (_, row) => amount(sumRow(row)) },
    { title: '已分配合计', key: 'balance', width: 215, fixed: 'right', align: 'center', render: (_, row) => {
      const allocated = sumRow(row), delta = Math.round((allocated - row.estimatedTotal) * 1000000) / 1000000
  ...
  return <section className="pms-resource-panel pms-resource-monthly" aria-label="当前版本月度投入">
    ...
    <Tabs className="pms-resource-year-tabs" activeKey={selectedYear} onChange={setYear} items={[
      ...
      ...complete.years.map(value => { const months = complete.months.filter(month => month.startsWith(`${value}-`)); const total = months.reduce((sum, month) => sum + complete.totals[month], 0); return { key: value, label: `${value} 年 · ${months.length} 个月 · ${formatPersonMonth(total)} 人月 / ${sumResourceCost(costs.totals, months).toFixed(2)} 万元` } }),
    ]} />
    <div className="pms-resource-view-stats">
      <div title="总投入 = 人力投入 × 费率 + 非人力投入（元）÷ 10,000"><span>{periodLabel}总投入</span><strong>{sumResourceCost(costs.totals, view.months).toFixed(2)} <small>万元</small></strong><span>合计 {formatPersonMonth(stats.total)} 人月{selectedYear !== 'all' && ` · 全周期 ${formatPersonMonth(complete.visibleTotal)} 人月`}</span></div>
    ...
    {view.months.length ? <>
      <div className="pms-resource-trend" tabIndex={0} aria-label="月度投入趋势，可横向滚动"><svg role="img" aria-label={`${version.versionNumber}月度${cost ? '费用' : '人力投入'}趋势`} viewBox={`0 0 ${chartWidth} 170`} style={{ width: chartWidth, minWidth: '100%' }}>
        ...
        {view.months.map((month, index) => { const value = totals[month], height = value / peak * 100, x = 40 + index * slot; return <g key={month}><title>{month}：{amount(value)} {unit}</title>
      ...
      <Table<ResourceMonthlyCostRow> className="pms-table pms-hr-investment-table pms-resource-monthly-table" size="small" rowKey="id" columns={columns} dataSource={cost ? costs.rows : view.rows} pagination={false} scroll={{ x: 575 + visibleYears.length * 110 + view.months.length * 110 }} locale={{ emptyText: '当前版本暂无部门月度投入' }}
        summary={() => <Table.Summary.Row><Table.Summary.Cell index={0} colSpan={2} align="center">合计</Table.Summary.Cell>
          {view.months.map((month, index) => <Table.Summary.Cell key={month} index={index + 2} align="center">{amount(totals[month])}</Table.Summary.Cell>)}
          {visibleYears.map((value, index) => <Table.Summary.Cell key={value} index={view.months.length + index + 2} align="center">{amount(sumResourceCost(totals, complete.months.filter(month => month.startsWith(`${value}-`))))}</Table.Summary.Cell>)}
          <Table.Summary.Cell index={view.months.length + visibleYears.length + 2} align="center">{amount(allocatedTotal)}</Table.Summary.Cell>
          <Table.Summary.Cell index={view.months.length + visibleYears.length + 3} align="center"><span className={Math.abs(allocatedTotal - estimatedTotal) >= 0.0005 ? 'pms-resource-difference' : ''}>{amount(allocatedTotal)}/{amount(estimatedTotal)}（{allocatedTotal > estimatedTotal ? '+' : allocatedTotal < estimatedTotal ? '-' : ''}{amount(Math.abs(allocatedTotal - estimatedTotal))}）</span></Table.Summary.Cell>
        </Table.Summary.Row>} />
