...
export function exportResourceVersion(projectName: string, version: ResourceVersion, rows: ResourceMonthlyRow[]) {
  const { view, milestones, departments, phaseColumns, moneyMonths, metadata, nonLaborRows } = buildResourceVersionExportData(projectName, version, rows)
  exportMultiSheet([
    ...
    {sheetName:'非人力投入',rows:nonLaborRows,columns:[{key:'secondaryDepartment',title:'二级部门'},{key:'tertiaryDepartment',title:'三级部门'},{key:'secondarySubject',title:'二级科目'},{key:'tertiarySubject',title:'三级科目'},{key:'estimatedInvestment',title:'合计（元）'},...moneyMonths.map(month=>({key:month,title:month+'（元）'}))]},
