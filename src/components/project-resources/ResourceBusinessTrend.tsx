...
export default function ResourceBusinessTrend({ trend }: { trend: DashboardBusinessTrend }) {
  ...
  const width = Math.max(820, periods.length * (grain === 'stage' ? 120 : grain === 'week' ? 72 : 64)), height = 248, left = 64, top = 22, baseline = 198
  ...
  return <>
    ...
    <div className="pms-dashboard-trend-scroll pms-dashboard-business-trend" tabIndex={0} aria-label="四类投入趋势，可横向滚动">
      <svg viewBox={`0 0 ${width} ${height}`} style={{ width, minWidth: '100%' }} role="img" aria-label={`四类${grainLabel}${unit}趋势`}>
        ...
        {periods.map((period, index) => <g key={period}><text x={x(index)} y={220} textAnchor="middle">{grain === 'stage' ? labels[index] : grain === 'week' ? period.slice(5) : period.slice(2)}</text>{grain === 'week' && <text x={x(index)} y={236} textAnchor="middle">{period.slice(0, 4)}</text>}</g>)}
    ...
    <details className="pms-dashboard-trend-detail"><summary>查看{grainLabel}趋势明细（{unit}）</summary>
      <Table className="pms-table" size="small" rowKey="period" pagination={{ pageSize: 12, showSizeChanger: false }} scroll={{ x: 760 }}
        ...
        columns={[{ title: grain === 'stage' ? '阶段' : grain === 'week' ? '周一～周日' : '月份', dataIndex: 'label', width: grain === 'week' ? 220 : 120 }, ...series.map(item => ({ title: item.label, dataIndex: item.key, align: 'right' as const, render: (value: number | undefined) => value === undefined ? '—' : value.toFixed(precision) }))]} />
    </details>
  </>
}
