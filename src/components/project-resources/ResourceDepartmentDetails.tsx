...
export default function ResourceDepartmentDetails({ details }: { details: Details }) {
  const columns: ColumnsType<ResourceDepartmentDetail> = [
    ...
    { title: <Tooltip title="项目核算 ÷ 累至今日预估投入，分别按人月和总费用计算；分子与项目核算列一致。">累至今日预算执行率</Tooltip>, key: 'toDateExecution', width: 160, align: 'right', render: (_, row) => rates(row.toDateExecution, row.toDateCostExecution) },
