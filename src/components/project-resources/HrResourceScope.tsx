...
export function createHrViewHook<T extends ViewState>(original: UseBoundStore<StoreApi<T>>) {
  function useView<U = T>(selector?: (state: T) => U): U {
    ...
    const merged = useMemo(() => {
      ...
      const visibleVersions = new Set(projects.flatMap(project => scope ? project.versions.map(version => version.id) : ['annual', 'projectEstimate', 'projectBudget'].map(type => getActiveHrVersion(project.versions, type)?.id)))
      ...
      if (scope) {
        for (const [key, value] of Object.entries(state)) {
          ...
          if (!['setVersionLocked', 'setVersionActive'].includes(key) && key.startsWith('set') && typeof value === 'function' && key !== 'setShowNewProjectModal') {
        ...
        for (const key of ['setVersionLocked','setVersionActive','addVersion','copyVersion','deleteVersion','updateVersion','updateVersionDepartmentInvestments','updateDepartmentInvestments']) {
