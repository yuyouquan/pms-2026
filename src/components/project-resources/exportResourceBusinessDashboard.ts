...
export function exportResourceBusinessDashboard(projectName: string, analyses: (DashboardAnalysis | undefined)[], accounting: AccountingAnalysis | undefined, filter: DashboardFilter, mode: 'labor' | 'cost', grain: DashboardTrendGrain, stages: readonly (DashboardStageDefinition | undefined)[] = [], details?: ResourceDepartmentDetails) {
  ...
  exportMultiSheet([
    ...
    { sheetName: '偏差及执行率', columns: [...contextColumns, { key: 'name', title: '指标' }, { key: 'formula', title: '计算口径' }, amount('percent', '百分比（%）', 2), amount('labor', '预估人月', 6), amount('cost', '预估人力费用（万元）', 2), amount('laborDelta', '人月差'), amount('costDelta', '费用差（万元）', 2)], rows: rowsWithContext([
      ...
      { name: '累至今日执行率', formula: '截至今日核算人月÷累至今日预估人月', percent: details?.total.toDateExecution, range: `截至 ${details?.today ?? '—'}` },
      { name: '全生命周期预算执行率', formula: '核算费用÷项目预算费用', percent: metrics.execution },
    ]) },
    ...
    { sheetName: '部门投入明细', columns: [...contextColumns, { key: 'primary', title: '一级部门' }, { key: 'secondary', title: '二级部门' },
      ...
      { key: 'today', title: '累至今日截止日期' }, amount('actualToDateLabor', '截至今日核算人月', 6), amount('toDateExecution', '累至今日执行率（%）', 2), amount('lifecycleExecution', '全生命周期预算执行率（%）', 2)],
      rows: rowsWithContext(details ? [...details.rows, details.total].map(row => ({ primary: row.primary, secondary: row.secondary, today: details.today,
        ...
        actualToDateLabor: row.actualToDate?.labor, toDateExecution: row.toDateExecution, lifecycleExecution: row.lifecycleExecution })) : []) },
    { sheetName: '口径与检查', columns: [...contextColumns, { key: 'title', title: '项目' }, { key: 'detail', title: '说明' }], rows: rowsWithContext([
      ...
      { title: '累至今日预估', detail: '依次选取正式项目预算、正式年度预算、正式项目概算的部门投入及里程碑，阶段内按自然日均摊；开始日为0，结束日为100%，同日开始结束按当天完成。忽略月度手动分配及顶部日期筛选，预估费用仅为人月×费率；缺失日期或阶段投入不平衡不计算。' },
