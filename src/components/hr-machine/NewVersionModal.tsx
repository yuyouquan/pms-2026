...
import { canCreateHrVersion, isHrVersionEditable, getHrVersionSeed, nextHrMinorVersion } from '@/lib/hrVersionRules'
...
export default function NewVersionModal({ open, projectId, versionId, embedded = false, fixedBudgetType, onSaved, onCancel }: { open: boolean; projectId: string; versionId?: string; embedded?: boolean; fixedBudgetType?: BudgetType; onSaved?: () => void; onCancel: () => void }) {
  ...
  const [budgetType, setBudgetType] = useState<BudgetType>(fixedBudgetType ?? 'annual')
  ...
  const canSave = editing ? Boolean(project && editingVersion && isHrVersionEditable(project, editingVersion) && canEditHrInScope(project, scopeId)) : canCreateHrVersion(project, budgetType)
  ...
  useEffect(() => {
    ...
    setBudgetType(projects.find(item => item.id === id)?.versions.find(version => version.id === versionId)?.budgetType ?? fixedBudgetType ?? getHrAllowedBudgetTypes(projects.find(item => item.id === id))[0] ?? 'annual')
  }, [open, projectId, scopeId, versionId, fixedBudgetType])
  useEffect(() => {
    if (!open) return
  ...
  return <HrVersionSurface embedded={embedded} className="pms-modal pms-hr-version-modal" title={<HrVersionModalTitle title={editing ? "编辑版本" : "新增版本"} projectName={project?.name} versionNumber={editingVersion?.versionNumber ?? (project ? `V0.${nextHrMinorVersion(project.versions, budgetType)}` : undefined)} versionLabel={editing ? "版本号" : "将创建版本"} />} open={open} onCancel={onCancel} onOk={handleOk} okText={editing ? "保存" : "创建"} cancelText="取消" width={1560} okButtonProps={{ disabled: !canSave }}>
    ...
    <Form layout="vertical">
      ...
      {!fixedBudgetType && <Form.Item label="预算类型" required>{editing ? <HrReadonlyField label="预算类型" value={budgetOptions.find(option => option.value === budgetType)?.label} reason="版本预算类型不可修改" /> : <Select value={budgetType} options={budgetOptions} onChange={setBudgetType} />}</Form.Item>}
