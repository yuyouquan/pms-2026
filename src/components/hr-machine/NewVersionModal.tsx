...
export default function NewVersionModal({ open, projectId, versionId, onCancel }: { open: boolean; projectId: string; versionId?: string; onCancel: () => void }) {
  ...
  return <Modal className="pms-modal pms-hr-version-modal" title={editing ? "编辑版本" : "新增版本"} open={open} onCancel={onCancel} onOk={handleOk} okText={editing ? "保存" : "创建"} cancelText="取消" width={1560} okButtonProps={{ disabled: !canSave }}>
    ...
    <Form layout="vertical">
      ...
      <Form.Item label={editing ? "版本号" : "将创建版本"}><HrReadonlyField label="版本号" value={editingVersion?.versionNumber ?? (project ? `V0.${nextHrMinorVersion(project.versions, budgetType)}` : '')} reason="版本号自动生成" /></Form.Item>
