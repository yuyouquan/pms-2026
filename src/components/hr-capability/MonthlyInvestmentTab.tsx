...
import { Card, Table, Button, Tooltip, Tag, Checkbox, Space, Input, Select, DatePicker } from 'antd'
...
import type { CapabilityMonthlyInvestment, BudgetType } from '@/types/hrCapability'
import { exportSheet, exportTimestamp, type ExportColumn } from '@/utils/exportExcel'

/** 表格行：在月度投入记录上附加项目信息，便于直接渲染 */
interface MonthlyInvestmentRow extends CapabilityMonthlyInvestment {
  projectName: string
}
...
export default function MonthlyInvestmentTab() {
  ...
  const setEditingMonthlyId = useHrCapabilityStore((s) => s.setEditingMonthlyId)
  ...
  // ── 筛选状态（多选，空数组=不筛选） ──────────────────────
  const ALL_BUDGET_TYPES: BudgetType[] = CAPABILITY_BUDGET_TYPES.map((t) => t.value)
  ...
  const budgetTypeOptions = useMemo(
    () => CAPABILITY_BUDGET_TYPES.map((t) => ({ label: t.label, value: t.value })),
    [],
  )
  ...
  const totalRow = useMemo(() => {
    ...
    for (const row of dataSource) {
      ...
      for (const monthKey of sortedMonths) {
        const val = row.monthlyData[monthKey] || 0
  ...
  const columns = useMemo<ColumnsType<MonthlyInvestmentRow>>(() => {
    const baseColumns: ColumnsType<MonthlyInvestmentRow> = [
      ...
      },
      {
        ...
        key: 'budgetType',
        width: 100,
        render: (value: CapabilityMonthlyInvestment['budgetType']) =>
          CAPABILITY_BUDGET_TYPE_LABELS[value] ?? value,
      },
      {
        title: '版本号',
  ...
  const handleExport = () => {
    const exportColumns: ExportColumn[] = [
      ...
      { key: 'secondaryDepartment', title: '二级部门', width: 12 },
      {
        ...
        title: '预算类型',
        width: 10,
        formatter: (value) => CAPABILITY_BUDGET_TYPE_LABELS[value as BudgetType] ?? String(value),
      },
    ...
    exportSheet(
      dataSource,
      exportColumns,
      `能力建设项目月度预估投入_${exportTimestamp()}.xlsx`,
      '月度预估投入',
    )
  }

  return (
    <div className="pms-capability-monthly-investment-tab">
      <Card
      ...
      <style jsx global>{`
        .pms-capability-monthly-investment-tab .pms-table {
          font-variant-numeric: tabular-nums;
        }
        /* 合计行：品牌色背景突出显示 */
        .pms-capability-monthly-investment-tab .pms-table .pms-summary-row > td {
          background: var(--pms-brand-surface) !important;
          color: var(--pms-brand-strong);
          font-weight: 600;
          border-top: 2px solid var(--pms-brand-border);
        }
        .pms-capability-monthly-investment-tab .pms-table .pms-summary-row:hover > td {
          background: var(--pms-brand-surface) !important;
        }
        /* 已锁定版本：绿色背景 */
        .pms-capability-monthly-investment-tab .pms-table .pms-row-locked > td {
          background: #f0f9eb !important;
        }
        .pms-capability-monthly-investment-tab .pms-table .pms-row-locked:hover > td {
          background: #e8f5e0 !important;
        }
        /* 未锁定版本：灰色背景 */
        .pms-capability-monthly-investment-tab .pms-table .pms-row-unlocked > td {
          background: #f5f5f5 !important;
        }
        .pms-capability-monthly-investment-tab .pms-table .pms-row-unlocked:hover > td {
