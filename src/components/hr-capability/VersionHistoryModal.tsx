...
import type { CapabilityVersionOperationType } from '@/types/hrCapability'
...
const OPERATION_LABELS: Record<CapabilityVersionOperationType, { label: string; color: string }> = {
...
export default function VersionHistoryModal({ open, versionId, onCancel }: VersionHistoryModalProps) {
  const { projects } = useHrCapabilityStore()
      ...
      {version ? (
        <div style={{ marginTop: 16 }}>
          ...
          >
            <span>
              项目名称：
              <strong style={{ color: 'var(--pms-text-primary)' }}>{version.project.name}</strong>
            </span>
            <span>
              预算类型：
              <Tag color={CAPABILITY_BUDGET_TYPE_COLORS[version.version.budgetType]} style={{ marginLeft: 4 }}>
                {CAPABILITY_BUDGET_TYPE_LABELS[version.version.budgetType]}
              </Tag>
            </span>
            <span>
              版本号：
                  ...
                  children: (
                    <div>
                      ...
                      >
                        ...
                        <span style={{ color: 'var(--pms-text-primary)', fontWeight: 500 }}>
                          {log.operator}
                        </span>
                        <span style={{ color: 'var(--pms-text-tertiary)', fontSize: 12 }}>
                          {log.timestamp}
                        </span>
                      </div>
