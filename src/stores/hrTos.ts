...
function generateDepartmentMonthlyRecords(
  ...
  return splits.map((split: DepartmentMonthlySplit, idx: number) => {
    ...
    const phases = Object.fromEntries(getResourceRatioFields('tos').map(field => [field.key, Number(department[field.key as keyof typeof department]) || 0]))
    return {
      ...
      allocationBasis: hrMonthlyAllocationBasis(department.estimatedInvestment, phases, getResourcePhaseRatios('tos', version, department)),
      ...
      addVersion: (projectId, form) => set((s) => {
        ...
        const newProjects = s.projects.map(p => {
          ...
          const newVersion: HrTosVersion = {
            ...
            milestones,
            nonLaborInvestment: validateNonLaborInvestment(form.nonLaborInvestment ?? cloneNonLaborInvestment(latest?.nonLaborInvestment), useHrConfigStore.getState().data.nonLaborSubject ?? [], latest?.nonLaborInvestment, Object.values(useHrConfigStore.getState().data).flat()),
        ...
        const newProjects = s.projects.map(p => {
          ...
          const newVersions = p.versions.map(v => {
            ...
            return normalizeHrEditedVersion({
              ...
              nonLaborInvestment: nonLaborInvestment ? validateNonLaborInvestment(nonLaborInvestment, useHrConfigStore.getState().data.nonLaborSubject ?? [], v.nonLaborInvestment, Object.values(useHrConfigStore.getState().data).flat()) : v.nonLaborInvestment,
