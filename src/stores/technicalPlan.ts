...
const toTemplateTask = (task: TechnicalStageTask): TechnicalTemplateTask => ({
  id: task.id,
  order: task.order,
  taskName: task.name || task.taskName || '',
  ...(task.parentId ? { parentId: task.parentId } : {}),
  responsible: '技术项目负责人', predecessor: '',
  planStartDate: task.planStartDate, planEndDate: task.planEndDate,
  estimatedDays: 0, actualStartDate: '', actualEndDate: '', actualDays: 0,
  status: '未开始', progress: 0, defaultRoadmap: Boolean(task.parentId),
})
...
export const INITIAL_TECHNICAL_PLANS: TechnicalPlansByKey = {
  '9:tdt': {
    ...
    versions: [
      { id: 'tech-9-v1', versionNo: 'V1', templateType: 'tdt', status: '已发布', publishedAt: '2026-01-05T00:00:00Z', tasks: TDT_STAGE_TASKS.map(toTemplateTask) },
      { id: 'tech-9-v2-draft', versionNo: 'V2', templateType: 'tdt', status: '修订中', tasks: TDT_STAGE_TASKS.map(task => toTemplateTask({ ...task, name: task.parentId ? task.name : `${task.name}（修订）` })) },
    ],
  },
}
...
const clonePlans = (plans: TechnicalPlansByKey): TechnicalPlansByKey => Object.fromEntries(
  Object.entries(plans).map(([key, plan]) => [key, {
    ...
    columnSettings: { order: [...plan.columnSettings.order], visible: [...plan.columnSettings.visible] },
...
export type CreateRevisionResult = { ok: true; versionId: string } | { ok: false; reason: 'draft-exists' | 'inactive' | 'incomplete-configuration' }
export type RevisionMutationResult = { ok: true } | { ok: false; reason: 'missing-instance' | 'missing-draft' }
...
export interface TechnicalPlanActions {
  ...
  updateCurrentTasks: (scope: TechnicalPlanScope, tasks: readonly TechnicalTemplateTask[]) => RevisionMutationResult
...
const mutateDraft = (state: TechnicalPlanState, scope: TechnicalPlanScope, mutation: 'publish' | 'cancel' | 'tasks', payload?: string | readonly TechnicalTemplateTask[]) => {
...
export function createTechnicalPlanStore(initial: Partial<TechnicalPlanState> = {}) {
  ...
  return {
    ...
    updateCurrentTasks: (scope: TechnicalPlanScope, tasks: readonly TechnicalTemplateTask[]) => { const output = mutateDraft(state, scope, 'tasks', tasks); update(output.state); return output.result },
...
const zustandActions = (set: (value: Partial<TechnicalPlanState> | ((state: TechnicalPlanState) => Partial<TechnicalPlanState>)) => void, get: () => TechnicalPlanState): TechnicalPlanActions => ({
  ...
  updateCurrentTasks: (scope, tasks) => { const output = mutateDraft(get(), scope, 'tasks', tasks); set(output.state); return output.result },
...
export const useTechnicalPlanStore = create<TechnicalPlanState & TechnicalPlanActions>()(persist(
  ...
  { name: 'pms-technical-plans', version: 1, storage: createJSONStorage(() => localStorage), partialize: state => ({ plansByKey: state.plansByKey }) },
))
