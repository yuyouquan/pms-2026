...
- Final review test gap closed: after switching to an unauthorized actor, the focused verifier now invokes a real `milestoneSchedule` patch with valid changed dates against an otherwise unlocked editable copy, asserts the permission error, and proves the complete project state is unchanged. It then restores the authorized actor, applies the identical patch successfully, and checks both saved anchors so no lock, scope, or data validation can mask the RBAC result. `node scripts/verify-budget-milestone-scheduling.mjs` exited 0 after this addition.
