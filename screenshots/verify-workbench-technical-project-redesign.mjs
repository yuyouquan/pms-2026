...
try {
  ...
  await runScenario('04 machine tos technical quick filters are linked', {}, async page => {
    ...
    console.log('  STEP verify quick-to-advanced mirror, then change advanced value')
    await clickAria(page, '筛选')
    await page.waitForFunction(() => (document.querySelector('[aria-label="品牌筛选值"]')?.textContent || '').includes('Infinix'))
    await selectAriaOption(page, '品牌筛选值', 'TECNO')
    await clickExact(page, '.pms-floating-config-popover button', '确认')
    await page.waitForFunction(() => (document.querySelector('[aria-label="快捷筛选-品牌"]')?.textContent || '').includes('TECNO'))
    await page.waitForFunction(() => {
      const rows = Array.from(document.querySelectorAll('.ant-table-tbody tr[data-row-key]')).filter(element => element.getBoundingClientRect().height > 0)
      return rows.length > 0 && rows.every(row => (row.textContent || '').includes('TECNO'))
    })
