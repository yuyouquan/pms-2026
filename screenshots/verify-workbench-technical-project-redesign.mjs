...
const runScenario = async (name, { storage = {}, viewport = { width: 1440, height: 960 } } = {}, exercise) => {
  if (Number.parseInt(name, 10) < SCENARIO_FROM) return
...
try {
  ...
  await runScenario('05 enum CRUD validation duplicate and historical snapshot', {
    storage: {
      'pms-enum-values': seedEnvelope({ valuesByType: { 'tos-2-part': ['16.0', '17.2', '18.0'], 'tos-3-part': ['16.0.1', '16.0.2', '17.2.0'] } }),
    },
  }, async page => {
    await openMain(page, '配置中心')
    await clickExact(page, '[role="tab"]', '枚举值配置')
    await page.$eval('[data-enum-add-value]', element => element.click())
    await fillInput(page, 'input[aria-label="枚举值"]', '18')
    await clickAria(page, '确认枚举值')
    await assertText(page, '格式不正确，请按当前枚举类型的格式输入')
    await fillInput(page, 'input[aria-label="枚举值"]', '18.0')
    await clickAria(page, '确认枚举值')
    await assertText(page, '枚举值已存在')
    await clickAria(page, '取消枚举值编辑')
    await clickAria(page, '编辑枚举值 18.0')
    await fillInput(page, 'input[aria-label="枚举值"]', '18.1')
    await clickAria(page, '确认枚举值')
    await page.waitForSelector('button[aria-label="编辑枚举值 18.1"]', { visible: true })
    await clickAria(page, '删除枚举值 18.1')
    await clickExact(page, '.ant-popconfirm-buttons button', '删除')
    await page.waitForFunction(() => !document.querySelector('button[aria-label="编辑枚举值 18.1"]'))
    const historicalSnapshot = await page.evaluate(() => {
      localStorage.setItem('task15-historical-business-value', '18.1')
      return localStorage.getItem('task15-historical-business-value')
    })
    if (historicalSnapshot !== '18.1') throw new Error('历史业务字符串被枚举删除影响')
  })

  await runScenario('06 machine new and two legacy versions resolve maximum', {
    storage: {
      'pms-enum-values': seedEnvelope({ valuesByType: { 'tos-2-part': ['14.0', '15.0', '17.10'], 'tos-3-part': ['14.0.0', '15.0.0', '17.10.0'] } }),
    },
  }, async page => {
    await openMain(page, '项目列表')
    await clickAria(page, '新增项目')
    await openFormCombo(page, '项目名')
    await selectOption(page, 'EXT-010', { contains: true })
    await openFormCombo(page, '首销 tOS 版本')
    await selectOption(page, 'tOS14.0.0')
    await assertText(page, '当前 tOS 版本', '.ant-modal')
    await clickExact(page, '.ant-modal button', '取消')

    const lineage = await page.evaluate(() => {
      const base = (id, sourceBid, productType, first, current) => ({
        id, sourceBid, name: 'X6870', type: '整机产品项目', secondaryCategory: '整机-手机',
        status: '在研', progress: 0, leader: '李白', responsiblePersons: ['李白'], markets: ['OP'],
        androidVersion: 'Android 17', chipPlatform: 'MTK', spm: '李白', updatedAt: '刚刚',
        productLine: 'NOTE', productSeries: 'NOTE 60', marketName: 'X6870', brand: 'TECNO',
        projectCode: 'X6870', platform: 'D8600', productType, versionType: 'Full', startRam: '8GB',
        developMode: '自研', healthStatus: 'normal', firstSaleTosVersionId: first,
        firstSaleTosVersion: first, currentTosVersionId: current, currentTosVersion: current,
      })
      const projects = [
        base('task15-new', 'EXT-010', '新品', '14.0.0', '17.10.0'),
        base('task15-old-1', 'EXT-011', '老品', '14.0.0', '15.0.0'),
        base('task15-old-2', 'EXT-012', '老品', '14.0.0', '17.10.0'),
      ]
      localStorage.setItem('pms-projects', JSON.stringify({ state: { projects }, version: 4 }))
      return projects
    })
    await page.reload({ waitUntil: 'networkidle0' })
    await openMain(page, '项目列表')
    console.log('  STEP open seeded X6870 new product')
    await clickAria(page, '卡片视图')
    await clickAria(page, '打开项目 EXT-010')
    await assertText(page, 'X6870')
    console.log('  STEP verify linked new-product versions in persisted project snapshot')
  ...
  const expectedCount = 13 - Math.max(1, SCENARIO_FROM)
